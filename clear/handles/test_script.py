...
class TestScriptHandler:
    ...
    async def add_test_script(request):
        ...
        job["directory"] = await TestScriptHandler.create_new_directory(request, job=job)
        job["file_path"] = await TestScriptHandler.create_script_file(request, job=job)
        job["created"] = datetime.now()

        added_data = deepcopy(job)
        added_data["db_id"] = await TestScriptHandler.add_script_data_to_db(request, data_to_add=job)

        return web.Response(text=dumps(added_data), content_type="application/json")
    ...
    async def add_script_data_to_db(self, data_to_add):
        ...
        try:
            ...
            logging.info("HHHHHHHHHHHHHHHHHHHHHAAAAAAAAAAAAAAAAAAAHHHHHHHHHHHHHHHHHHAAAAAAAAA")
    ...
    async def create_new_directory(self, job=None):
        ...
        try:
            ...
            logging.exception("[{uid}] Failed to create directory for script: {e}".format(uid=job["uid"], e=e))

        else:
            logging.info(logger_msg["parser_folder_created"].format(uid=job["uid"], foldername=dir_path))
    ...
    async def show_test_scripts(request):
        data_to_return = await TestScriptHandler.get_scripts_data_from_db(request)
        return web.Response(text=dumps(data_to_return), content_type="application/json")
    ...
    async def get_script_data_by_id(request):
        test_script_id = request.match_info["ts_id"]
        agent = DatabaseAgent()
        return agent.find("test_scripts", {"_id": ObjectId(test_script_id)})
    ...
    async def get_script_data(request):
        script_data = await TestScriptHandler.get_script_data_by_id(request)
        json_script_data = dumps(script_data)
        return web.Response(text=json_script_data, content_type="application/json")
